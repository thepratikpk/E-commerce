...
import { assets, productScreenshots } from '../assets/assets';
        ...
        <div>
          {
            cartData.map((item,index)=>{
              const productData=productScreenshots.find((productScreenshots)=>productScreenshots._id===item._id);
              return (
                <div key={index} className='py-4 border-b border-t text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                  ...
                  <input onChange={(e)=>e.target.value==='0'?null:updateQuantity(item._id,item.size,Number(e.target.value))} type="number" min={1} defaultValue={item.quantity} className='border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1' />
